...
\section{Cross-Entropy Monte Carlo algorithm}
        ...
        \hspace{1cm}$burn.in$ is a burn-in parameter indicating that $burn.in*thin$ initial
        realizations of the Markov chain are to be discarded
        (typically initial values of the Markov chain are not valid
        because the chain is not stable yet and sensitive to
        the selection of the initial value). The default value is $burn.in=N$, meaning that
        half of the chain is discarded.}
        \item{\textbf{Step 2: Proposal.} Let $u$ be an integer
        uniform variable defined on a discrete space $[1, min(k+1, n-k+1)]$.
        \begin{itemize}
        \item{At each step $b$, choose a realization of $u$ from the uniform distribution
        defined above.}
        \item{Let $x=x^{(b)}$. Perform $u$ swaps of 0's and 1's in $x$ subject to the constraints defined before. 
        Propose the generated $x$ as the next realization of the chain.}
        \end{itemize}}
        \item{\textbf{Step 3: Acceptance.} The proposed realization from Step 2 is
        accepted as the next realization of the Markov chain if the
        acceptance probability $p_A$ defined by
        \begin{eqnarray*}
        p_A&=&\frac{P_v(x)}{P_v(x^{(b)})} \\
         &=&\prod_{r=1}^{k}\prod_{j=1}^n
         (p_{jr})^{x_{jr}-x_{jr}^{(b)}}
        \end{eqnarray*}
        is greater than $y$, where $y$ is an independently generated random variable from $U(0,1)$. Otherwise, $x$
        is rejected and $x^{(b+1)}=x^{(b)}$.}
        \end{enumerate}
